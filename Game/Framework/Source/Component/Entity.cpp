...
Entity::Entity()
    : m_position(XMFLOAT3(0, 0, 0)), m_rotate(XMFLOAT3(0, 0, 0)), m_scale(XMFLOAT3(1, 1, 1)),
    m_tag(""), m_name(""), m_isActive(false) {}

Entity::Entity(const XMFLOAT3& position, const XMFLOAT3& rotate, const XMFLOAT3& scale,
               const std::string& tag, const std::string& name)
    : m_position(position), m_rotate(rotate), m_scale(scale), m_tag(tag), m_name(name),
    m_isActive(false) {}
